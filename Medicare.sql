...
SELECT apc,
      ...
      ROUND(AVG(average_total_payments), 2) AS avg_payments_by_apc,
      ROUND(SUM(outpatient_services) * AVG(average_total_payments), 2) AS total_revenue
FROM bigquery-public-data.cms_medicare.outpatient_charges_2015
GROUP BY apc
ORDER BY num_claims DESC
LIMIT 10;

-- Provder revenue by state
...
-- Medicare spending by state VS state GDP
WITH acs_state AS (
  SELECT
    CASE
...
SELECT m.provider_state,
      m.avg_payments_by_state,
      m.total_revenue, -- is also a total spending by medicare
      (a.population * a.income_per_capita) AS estimated_gdp, --
      total_revenue / (a.population * a.income_per_capita) *100 AS revenue_gdp_ratio
FROM medicare_spending AS m
JOIN acs_state AS a
ON m.provider_state = a.state
ORDER BY 5 DESC;
